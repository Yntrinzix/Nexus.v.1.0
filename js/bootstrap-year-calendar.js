 ...
 * ========================================================= */
 ...
 (function($) {
	...
	var colors = $.fn.calendar.colors = ['#2C8FC9', '#9CB703', '#F5BB00', '#FF4A32', '#B56CE2', '#45A597','#c356a1','#ff8a00','#828282','#b9f8ff','#d7ffb9'];
 ...
 //added by John Mark Martinez Mar 10 2017
 
 
 //Script for Managers---------------------------------------------------------------------------------------------------------------------------------
 
var modalTitle=1;
var loginType = 1; //1 = manager 0 = employee 

function report(){
	
	document.getElementById('myReportModal').style.display= 'inline';
	document.getElementById('myMainModal').style.display= 'none';
	if(loginType ==1){
		document.getElementById('myFilterModal').style.display= 'none';
	}
	$('#event-modal').modal();
	console.log("clicked");
...
function filter(){
	...
	document.getElementById('myFilterModal').style.display= 'inline';
	document.getElementById('myReportModal').style.display= 'none';
	document.getElementById('myMainModal').style.display= 'none';
	$('#event-modal').modal();
...
function filterProcess(){
	...
	console.log("before");
	$.getJSON("https://vjnexus.000webhostapp.com/js/task-view-mng.json",function(response){
		console.log(response+" powerful men");
		console.log("inside");
		myData=response;
		
	});
	
	var user = document.getElementById("employee-filter").selectedIndex;
	
	$.ajaxSettings.async = true;
	console.log(user+1);
	
	for(i=0 ; i < myData.length ; i++){
						if(user+1==myData[i].idEmp){
							var event = {
									
											id: myData[i].id,
											name: myData[i].name,
											location: myData[i].location,
											startDate: new Date(myData[i].startYear, (myData[i].startMonth-1), myData[i].startDay),
											endDate: new Date(myData[i].endYear,  (myData[i].endMonth-1),  myData[i].endDay),
											idAssigned:myData[i].idAssigned
...
function editEvent(event) {
	console.log(loginType);
	document.getElementById('myReportModal').style.display= 'none';
	document.getElementById('myMainModal').style.display= 'inline';
	if(loginType ==1){
		document.getElementById('myFilterModal').style.display= 'none';
	}
	if(modalTitle==1){
		...
		if(loginType ==1){
			document.getElementById('done-event').style.display = 'none';
			document.getElementById('update-event').style.display= 'none';
			document.getElementById('save-event').style.display= 'inline';
		}
		document.getElementById("mod-title").innerHTML = "Task Manager";
	}
	else if(modalTitle==0) {
		
		if(loginType ==1){
			document.getElementById('done-event').style.display= 'inline';
			document.getElementById('update-event').style.display= 'inline';
			document.getElementById('save-event').style.display= 'none';
		}
		document.getElementById("mod-title").innerHTML = "Update Task";
    ...
    $('#event-modal textarea[name="event-location"]').val(event ? event.location : '');
    $('#event-modal input[name="event-start-date"]').datepicker('update', event ? event.startDate : '');
    $('#event-modal input[name="event-end-date"]').datepicker('update', event ? event.endDate : '');
	$('#event-modal input[name="idAssigned"]').val(event ? event.idAssigned : '');
    $('#event-modal').modal();
	modalTitle=1;
}
...
function saveEvent() {
	...
	if($('#event-modal input[name="event-name"]').val()!= ''&& $('#event-modal option[id="employee"]').val()!= ''){
		console.log("possitive");
		var event = {
			
			id: $('#event-modal input[name="event-index"]').val(),
			name: $('#event-modal input[name="event-name"]').val(),
			location: $('#event-modal textarea[name="event-location"]').val(),
		...
		else
		{
			...
			$.get("https://vjnexus.000webhostapp.com/dbmanager/task-json-extract_pdo.php");
			$.get("https://vjnexus.000webhostapp.com/dbmanager/task-json-extract_pdo.php");
			return false;
			$.ajaxSettings.async = true;
		...
		alert("Data Unsaved, Task Name is Empty or There are no assigned employee");
...
function loadJASON() {
	var dataSource = $('#calendar').data('calendar').getDataSource();
	dataSource = [];
	var userID = document.getElementById("username").getAttribute("tag");//get user ID	
	...
	$.ajaxSettings.async = true;
		...
		console.log(userID);
				...
				if(userID==1){
					...
					dataSource = $('#calendar').data('calendar').getDataSource();
					if(i==0){
						console.log(myData[i].name);
						dataSource.push(event);
					}
					else if(i>0){
						if(myData[i].id!==myData[i-1].id){
						...
						$('#calendar').data('calendar').setContextMenuItems(managerMenu);
						$('#calendar').data('calendar').setEnableRangeSelection(true);
						loginType = 1;
						}
					}
					...
					for(i=0 ; i < myData.length ; i++){
						...
						$('#calendar').data('calendar').setContextMenuItems(managerMenu);
						$('#calendar').data('calendar').setEnableRangeSelection(false);
						loginType = 0;
						}
					}
				}
